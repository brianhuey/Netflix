...
def get_transform_for_features(feature_name_list, pos_dist_file_list, neg_dist_file_list, pos_prior=.5, tail_prune = .05,n_bins=20):
    
    results = []
    for i in xrange(len(feature_name_list)):
        
        (pos_pts,min_val,max_val) = read_dist(pos_dist_file[i],bins=n_bins,prune=tail_prune)
        (neg_pts,min_val,max_val) = read_dist(neg_dist_file[i],bins=n_bins,prune=tail_prune,max_val=max_val,min_val=min_val)
...
#print "min,max", min_val,max_val
   ...
   # print "neg_density", neg_density
    ...
    width = (max_val-min_val)/n_bins
        ...
        for b in n_bins:
            pred_and_POS = float(pos_density[b]* pos_prior)
            pred_and_NEG = float(neg_density[b]* neg_prior)
           
            distribution[b] = (pred_and_POS/(pred_and_POS + pred_and_NEG)) 
            
    results.append((feature_name_list[i],distribution))
        
    return results, max_val, min_val, n_bins, neg_bins, pos_bins 


def get_probs_from_feature(test_feature_file, probs_file, outfile_name='./transformed.features'):
