...
def evaluate_gradients(A,U,V,p,p_sample_size,hard=False,random_pairs = False):
    (n,m) = A.shape
    data_mse = 0.0
    rnd_mse = 0.0
    count = 0
    ...
    data_count = count
    print ("data_rmse ", math.sqrt(data_mse/(1.0*(data_count))))
    ...
    if not random_pairs:
        ...
        for i in xrange(p_sample_size):
            ...
            if (row,col) in index_hash:
                continue
            random_pair_count += 1
            random_pairs[(row,col)] = True
        print "random_pair_count ", random_pair_count

    for (row,col) in random_pairs.keys():
    ...
    print ("rnd_rmse ", math.sqrt(rnd_mse/(1.0*(count-data_count))))
    return ((data_mse+rnd_mse)/(1.0*count),del_U,del_V,random_pairs)
...
def hack_nmf_iter(A,k,p,p_sample_size,hard=False):
    ...
    for i in xrange(0,iterations):
        ...
        (error,del_U,del_V,random_pairs) =  evaluate_gradients(A,U,V,p,p_sample_size,hard=hard,random_pairs = random_pairs)
...
def test_nmf_iter(n,m,k,p,binary=False):
    ...
    (error,del_U,del_V,random_pairs) =  evaluate_gradients(A,U,V,p,100)
...
def k_types_test_nmf(n,m,k,dn,dm):
    ...
    (error,del_U,del_V,random_pairs) =  evaluate_gradients(A,U,V,0,A.nnz,False)
